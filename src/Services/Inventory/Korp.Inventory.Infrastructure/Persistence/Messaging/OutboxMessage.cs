...
public sealed class OutboxMessage
{
    ...
    {
        if (lockId == Guid.Empty || lockedUntilUtc <= NextAttemptAtUtc || PublishedAtUtc is not null)
        {
            throw new InvalidOperationException("Outbox lease is invalid.");
        }
    ...
    {
        if (LockId is null || publishedAtUtc < OccurredAtUtc)
        {
            throw new InvalidOperationException("Publisher confirmation requires an active lease.");
        }
